...
module.exports.requestHosting = function(port, args, socket, cb)
{
  ...
  setTimeout(function()
  {
    host(port, args, socket, function(err)
    {
      if (err)
      {
        rw.logError({game: games[port].name, port: port, args: args}, `host Error:`, err);
        cb(err, null);
        return;
      }

      //remove the request, as it has finished
...
module.exports.deleteGameSavefiles = function(data, cb)
{
  kill(games[port].instance, function(err)
  {
    if (err)
    {
      rw.logError({game: games[port].name, data: data}, `kill Error:`, err);
      cb(err, null);
      return;
    }
...
function assignHandler(game, port)
{
  switch(game.gameType.toLowerCase())
  {
    ...
    default:
    rw.writeToGeneralLog(`assignHandler() Error: Game ${game.name} has an incorrect game type, cannot assign a handler: ${game.gameType}.`);
    throw `The game ${game.name} has an incorrect game type, cannot assign a handler: ${game.gameType}.`;
  }
}

function host(port, args, socket, cb)
{
  handlers[port].call.host(port, args, games[port], function(err, instance)
  {
    if (err)
    {
      rw.logError({port: port, args: args, game: games[port].name}, `host Error:`, err);
      cb(err);
      return;
    }
    ...
    {
      instance = null;
      socket.emit("gameError", {name: games[port].name, error: err});
    });
    ...
    instance.on("close", (code, signal) =>
    {
      ...
      {
        socket.emit("gameClosedUnexpectedly", {name: games[port].name});
      }
    ...
    if (instance.stderr != null)
    {
      ...
      {
        instance = null;
        socket.emit("gameError", {name: games[port].name, error: data.toString()});
      });
    }
